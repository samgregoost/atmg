...
def interpolate(model, feats: torch.Tensor, root_feat: torch.Tensor, steps: int):
    ...
    interp_feats = [
            torch.lerp(feats*1/2, 3.*feats, weight.item())
...
def get_image_feat(model, image_feats, text_feat):
    scores = L.pairwise_oxy_angle(image_feats, text_feat, model.curv.exp())
    print(scores.shape)
    return scores
...
def main(_A: argparse.Namespace):
    ...
    loader = DataLoader(
        DatasetCatalog.build(
            'imagenet', 'datasets/eval', "test", image_transform
        ),
        batch_size=128,
        num_workers=4,
    )
   ...
   # image_feats_e, labels, all_images = _encode_dataset(loader, model, project=False)
    ...
    returned_image = all_images[returned_image_idx.item()]
 ...
 #   for i in range(40):
  ...
  #  interp_feats = torch.cat(interp_feat_list, dim=0)
   ...
   # interp_feats = L.exp_map0(interp_feats, model.curv.exp())
    ...
    interp_feats = interpolate(model, image_feats[image_index][None, ...], root_feat, 40)
    nn1_scores = calc_scores(model, interp_feats, image_feats, has_root=True)
    nn1_scores, _nn1_idxs = nn1_scores.max(dim=-1)
    nn1_images = [all_images[_idx.item()] for _idx in _nn1_idxs]
