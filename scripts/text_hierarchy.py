...
def get_text_feats(model: MERU | CLIPBaseline) -> tuple[list[str], torch.Tensor]:
    ...
    noun_prompt_tokens = tokenizer(pexels_text["nouns"])
    noun_feats = model.encode_text(noun_prompt_tokens, project=True)

    adj_prompt_tokens = tokenizer(pexels_text["adjectives"])
    adj_tokens = model.encode_text(adj_prompt_tokens, project=True)

    all_text_feats = [caption_feats, noun_feats, adj_tokens]
    all_pexels_text = [
...
def main(_A: argparse.Namespace):
    ...
    text_pool, text_feats = get_text_feats(model)
    k = 0
    for i in range(1000):
        e1 = random.choice(text_feats[0])
        e2 = random.choice(text_feats[1])
        e3 = random.choice(text_feats[2])
